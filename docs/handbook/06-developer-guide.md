...
metaphor migration generate Invoice edi
metaphor migration run
...
(`edi` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed edi          # run Rust seeders
metaphor migration generate-seeds edi  # emit SQL seed files
```
